...
# <3 title: StickManager
# <3 pic: https://img.icons8.com/fluency/48/000000/sticker.png
# <3 desc: Менеджер стикеров, поддерживающий видеостикеры с юзер-френдли дизайном
...
from telethon.tl.functions.messages import GetStickerSetRequest, InstallStickerSetRequest, UninstallStickerSetRequest, UploadMediaRequest
import asyncio
import os
...
class StickManagerMod(loader.Module):
    ...
    async def prepare_vid(self, message: Message) -> InputDocument:
        ...
        if w > h:
            clip = clip.resize(width=512, height=int(h * (512 / w)))
        else:
            clip = clip.resize(width=int((w * (512 / h))), height=512)

        clip.write_videofile("sticker.webm", audio=False, codec='libvpx-vp9', logger=None, fps=25)
        clip.close()
        ...
        # file = await self.client.upload_file(.read())
        # logger.info(file)
        # file = InputMediaUploadedDocument(file, mime, [])
        # document = await self.client(UploadMediaRequest(InputPeerSelf(), file))
        # document = get_input_document(document)

        return open('sticker.webm', 'rb')
    ...
    async def newpackcmd(self, message: Message) -> None:
        ...
        if not args or len(args) < 2:
            await utils.answer(message, self.strings('error').format('Usage: .newpack &lt;short_name&gt; &lt;name&gt;'))
            return

        if not reply or not reply.media:
            await utils.answer(message, self.strings('error').format('Reply to a media required'))
            return
        ...
        async with self.client.conversation('@stickers') as conv:
            try:
                ...
                if 'Now choose a name' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Error while creating pack')
                ...
                if 'Now send me' not in r.raw_text:
                ...
                if 'Now send me an emoji' not in r.raw_text:
                ...
                if 'provide a short name' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - No shortname option')
                ...
                if 'this short name is already taken' in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Occupied shortname')

                if 'Kaboom' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Unknown confirmation error (Kaboom)')
        ...
        await self.client(InstallStickerSetRequest(
        ...
        await utils.answer(message, self.strings('created').format(emoji, name, shortname))
    ...
    async def newvidpackcmd(self, message: Message) -> None:
        ...
        if not args or len(args) < 2:
            await utils.answer(message, self.strings('error').format('Usage: .newvidpack &lt;short_name&gt; &lt;name&gt;'))
            return

        if not reply or not reply.media:
            await utils.answer(message, self.strings('error').format('Reply to a photo required'))
            return
        ...
        async with self.client.conversation('@stickers') as conv:
            try:
                ...
                if 'A new set' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Error while creating pack')
                ...
                if 'Now send me the video' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Error while entering name')
                ...
                if 'Now send me an emoji' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Error while sending file')
    ...
    async def syncpackscmd(self, message: Message) -> None:
        ...
        async with self.client.conversation('@stickers') as conv:
            ...
            packs = []
            for row in [[btn.text for btn in row.buttons] for row in r.reply_markup.rows]:
                for btn in row:
                    packs += [btn]
                    if btn in self.stickersets:
                        continue

                    stickerset = await self.client(GetStickerSetRequest(stickerset=InputStickerSetShortName(btn), hash=round(time.time())))
        ...
        if len(args) == 1:
            for shortname, info in self.stickersets.items():
                if info['alias'] == args[0]:
                    ...
                    self.set('stickersets', self.stickersets)
                    await utils.answer(message, self.strings('alias_removed').format(args[0]))
                    return

            await utils.answer(message, self.strings('remove_alias_404').format(args[0]))
            return
        else:
            alias, pack = args
            ...
            self.set('stickersets', self.stickersets)
            await utils.answer(message, self.strings('created_alias').format(self.stickersets[pack]['emoji'], self.stickersets[pack]['title'], alias))
    ...
    async def stickdefcmd(self, message: Message) -> None:
        ...
        await utils.answer(message, self.strings('default').format(pack['emoji'], pack['title']))
    ...
    async def rmpackcmd(self, message: Message) -> None:
        ...
        try:
            await self.client(UninstallStickerSetRequest(
                stickerset=InputStickerSetShortName(pack['shortname'])
            ))
        except RPCError:
            await utils.answer(message, self.strings('pack404').format(pack['shortname']))
            return
        ...
        async with self.client.conversation('@stickers') as conv:
            try:
                ...
                if 'Choose the sticker set' not in r.raw_text:
                ...
                if 'you selected the set' not in r.raw_text:
                ...
                if 'The sticker set is gone' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Confirmation did not work')
        ...
        self.set('stickersets', self.stickersets)
        await utils.answer(message, self.strings('packremoved').format(pack['emoji'], pack['title']))
    ...
    async def unstickcmd(self, message: Message) -> None:
        ...
        async with self.client.conversation('@stickers') as conv:
            try:
                ...
                if 'Choose a sticker' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Error while starting action')

                await m.delete()
                await r.delete()

                m = await self.client.forward_messages('@stickers', [reply.id], message.peer_id)
                r = await conv.get_response()

                if 'I have deleted that sticker' not in r.raw_text:
        ...
        await utils.answer(message, self.strings('stickrm').format(random.choice(self.emojies)))
    ...
    async def stickcmd(self, message: Message) -> None:
        ...
        reply = await message.get_reply_message()
        if not reply or not reply.media:
            await utils.answer(message, self.strings('error').format('Reply to sticker required'))
            return
        ...
        if not pack:
            await utils.answer(message, self.strings('error').format('Default pack doesn\'t exist'))
            return
        ...
        if reply.media.document.mime_type.startswith('video'):
            if 'video' not in pack:
                pack = [self.find(_) for _, p in self.stickersets.items() if 'video' in p]
                if not pack:
                    await utils.answer(message, self.strings('error').format('Can\'t add video sticker - no video stickersets'))
                    return
        ...
        async with self.client.conversation('@stickers') as conv:
            try:
                ...
                if 'Choose a sticker set' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Error while starting action')
                ...
                if 'Alright!' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Error while choosing pack')
                ...
                if 'There we go' not in r.raw_text:
                    raise HikariException('UNEXPECTED_ANSWER - Error while sending emoji')
        ...
        await utils.answer(message, self.strings('kang').format(pack['emoji'], pack['shortname']))
        await asyncio.sleep(3)
        await message.delete()
